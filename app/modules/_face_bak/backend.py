...
class FaceModule:
    ...
    def _detect_with_onnx(self, bgr, width, height):
        """ONNX 分支：YuNet 检测 + 几何 478 点 + 椭圆掩膜"""
        ...
        try:
            ...
            for d in dets:
                ...
                faces.append(pts478)
                # 侧脸估计
                is_profile = self._estimate_profile(pts478, width, height)
                profiles.append(is_profile)
                # 轮廓：用几何生成
                # 简化：直接用 pts478 前 120 点作 outline
                outline = {"left": pts478[0:9], "right": pts478[9:18], "jaw": pts478[18:21]}
                outlines.append(outline)
            # 掩膜：批量生成
            h, w = bgr.shape[:2]
            # 用 onnx_det 的分割（椭圆）
            _, body_skin = onnx_det.segment_masks(bgr, faces)
            # 脸部皮肤：逐脸生成
            face_skin = np.zeros((h, w), dtype=np.uint8)
            import cv2 as _cv2
            for pts in faces:
                poly = np.array([(int(x*w), int(y*h)) for x, y in pts[:120]], dtype=np.int32)
                if len(poly) >= 3:
                    _cv2.fillPoly(face_skin, [poly], 1)
            # 转为 PIL 掩膜后续编码
    ...
    def detect(self, photo_id, force=False):
        ...
        # 优先 ONNX（25MB，共享 onnxruntime），失败回退 mediapipe（300MB）
        onnx_det = self._get_onnx_detector()
        if onnx_det is not None:
            try:
                ...
                if onnx_res:
                    ...
                    skin_mask_fm = self._face_skin_mask_from_landmarks(faces, width, height, _Img2) if faces else np.zeros((height, width), dtype=np.uint8)
                    skin_mask_b64 = self._encode_mask_png(skin_mask_fm, _Img2)
                    lip_mask_fm = self._face_lip_mask_from_landmarks(faces, width, height, _Img2) if faces else np.zeros((height, width), dtype=np.uint8)
                ...
                _log(f"ONNX 分支异常回退 mediapipe: {exc}")
    ...
    def plugin_status(self):
        ...
        mode = "onnx" if has_onnx else ("mediapipe" if has_mp else "missing")
        return {
            "success": True,
            "available": available,
            "mode": mode,
