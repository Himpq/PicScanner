...
<template>
  <div class="ps-gallery-shell">
    <!-- 侧边栏：复用全局 side 样式，分类面板已 Vue 化 -->
    <aside class="side">
      <div class="brand-block">
        <div class="side-path">{{ currentSource || '未选择来源' }}</div>
      </div>
...
.ps-gallery-shell > .side { width: 240px; flex-shrink:0; }
.ps-gallery-shell > .gallery-wrap { flex:1; min-width:0; }
.ps-gallery-shell > .date-rail { width: 160px; flex-shrink:0; }
.time-range-mirror { font-size:12px; color:var(--muted, #9aa0a6); padding:4px 8px; }
@media (max-width: 900px) {
  .ps-gallery-shell { flex-direction:column; }
  .ps-gallery-shell > .side, .ps-gallery-shell > .date-rail { width:100%; }
}
</style>
