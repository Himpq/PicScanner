...
// --- 1) 列数与 CSS auto-fill 一致 ---
// gridWidth 1200, item 168, gap 10 -> floor((1200+10)/178) = 6
assert.equal(L.columnsFor(1200, 168), 6);
// item 缩小到 112 -> floor(1210/122) = 9
assert.equal(L.columnsFor(1200, 112), 9);
// 极窄容器不应退化为 0 列
assert.equal(L.columnsFor(10, 168), 1);

// --- 2) 单分区高度 = header + ceil(n/cols) 行 * itemSize + (行-1) * gap ---
{
...
// --- 3) 前缀和必须连续，总高等于各分区之和 ---
{
  ...
  for (const s of m.sections) {
    assert.equal(s.top, acc, '分区 top 必须紧接上一个分区');
    acc += s.height;
  }
  assert.equal(m.totalHeight, acc);
}

// --- 4) 可视区间必须覆盖视口，且不能渲染整组（1806 张不能全上）---
{
  const m = mk(168);
  ...
  assert.ok(sec.start <= 1 && sec.end >= 2, '可视分区区间应覆盖第 2 个分区');
  ...
  // 1806 张、6 列、视口 900px 高 -> 约 6 行 * 6 列 + 缓冲，明显小于 100
  assert.ok(item.end - item.start < 100, '可视条目数应远小于总数，实际 ' + (item.end - item.start));
  // 光标所在行必须在区间内
  assert.ok(item.start <= 20 * 6 && item.end > 20 * 6, '可视区间必须包含光标所在行');
}

// --- 5) 缩放锚点：改变 itemSize 后，锚点照片在视口中的偏移保持不变 ---
{
  const before = 168;
  const after = 240;
  const m0 = mk(before);
  ...
  const viewportH = 900;
  // 取视口中心作为锚点
  const anchor = L.anchorAtPoint(m0, { scrollTop, pointOffset: viewportH / 2, itemSize: before });
  assert.ok(anchor, '应能定位到锚点');
  ...
  // 行对齐后允许一行以内的误差（照片在行内会随列数变化而左移，这是预期行为）
  assert.ok(drift <= after + GAP, `缩放后锚点漂移应在一行内，实际 ${drift.toFixed(1)}px`);
}

// --- 6) anchorAtPoint 的 offsetInViewport 必须等于传入的 pointOffset ---
{
  const m = mk(168);
...
console.log('P2 高度模型：全部 6 组断言通过');
console.log('  1200px 宽 / 168px 条目 ->', L.columnsFor(1200, 168), '列；1806 张 ->',
  L.dateSectionHeight({ count: 1806, itemSize: 168, gridWidth: 1200 }).height, 'px 高');
